...
const renderShows = function(responseArray) {
	...
	responseArray.forEach(function(artistInfo) {
		...
		$('#shows').prepend(
			`<button class="btn-default btn-secondary show-button btn-outline-secondary btn-block btn" data-artist="${artistInfo.lineup}"> <b>Date:</b> ${res}  <b>City:</b> ${artistInfo.venue.city}  <b>State:</b> ${artistInfo.venue.region}  <b>Lineup:</b> ${artistInfo.lineup}</button>`
		);
	});
};
